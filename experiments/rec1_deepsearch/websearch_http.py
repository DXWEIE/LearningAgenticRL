...
class SearchSQLCache:
    ...
    _instance = None
    DB_FILE = "search_cache.db"  # 缓存数据库文件
    ...
    def set(self, query: str, result: str):
        ...
        with self._lock, sqlite3.connect(self.DB_FILE) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "REPLACE INTO search_cache (query, result) VALUES (?, ?)",
                (query, result)
            )
            conn.commit()
...
def aggregate_search(query, top_k=11):
    ...
    if cached_result is not None:
        #print(f"[缓存命中] query: {query}")  # 日志可删除
    ...
    if unique_results and len(unique_results) > 0:
        ...
        return_content = f"No results found for '{query}'. Try with a more general query, or remove the year filter."

    # ===================== 缓存写入：执行完搜索后存入 =====================
    search_cache.set(query, return_content)
    #print(f"[缓存写入] query: {query}")  # 日志可删除
...
def rollout_search(params: Union[str, dict], **kwargs) -> str:
    try:
        query = params["query"]
    except:
