...
def visit_tool(params):
    target_url = params.get("url", "")
    goal = params.get("goal", "请总结网页内容")
    url = "http://xxxx:5123/visit"
    ...
    # 捕获所有网络异常：超时、连接失败、服务异常等
    except RequestException as e:
        err_msg = str(e).lower()
        if "timed out" in err_msg:
            print(f"[visit] Request timed out after {timeout_seconds}s")
            return f"[visit] Request timed out after {timeout_seconds}s."
        else:
            print(f"[visit] Connection failed: {e}")
            return "[visit] Failed to read page."
        

def simulate_visit_tool(params):
    target_url = params.get("url", "")
    goal = params.get("goal", "请总结网页内容")
    snippet = params.get("snippet", "")
    url = "http://xxx:5123/visit"
...
# # 示例用法
# summary = call_visit_service("https://arxiv.org/abs/2301.12345", "返回摘要内容")

# def search_tool(params):
#     url = "http://xxx:5123/search"
#     payload = {"query": params.get("query", "")}
#     response = requests.post(url, json=payload)
#     if response.status_code == 200:
#         return response.json().get('data')
#     else:
#         return "[search] Failed to retrieve search results."
...
def search_tool(params):
    ...
    # 捕获：超时、连接失败、网络错误、状态码非200
    except RequestException as e:
        if "timed out" in str(e).lower():
            return f"[search] Request timed out after {timeout_seconds}s."
        return f"[search] Failed to retrieve search results: {str(e)}"
...
class DeepSearchScheduler(MultiTurnScheduler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._simulate_visit_tool = simulate_visit_tool
        self.tools = {
            'search': self._search_tool,
            #'visit': self._visit_tool
            'visit': self._simulate_visit_tool
        }
        self.search_result_last_round = None


    def parse_search_result(self,text):
        # 匹配每个完整的搜索块：从 "A Google search for ..." 到下一个类似开头或结尾
        blocks = re.findall(
            r"A Google search for '(.*?)' found \d+ results:.*?(?=\n=======\nA Google search for|\Z)",
            text,
            re.DOTALL
        )
        ...
        all_queries = []
        for block in blocks:
            query = block.split('\n', 1)[0]  # 实际上 query 已经在 findall 中提取了，这里 block 是剩余内容
            # 但我们已经在 findall 的 group(1) 中拿到 query，所以需要调整

        # 更清晰的做法：同时捕获 query 和 body
        pattern = r"A Google search for '(.*?)' found \d+ results:(.*?)(?=\n=======\nA Google search for|\Z)"
        matches = re.findall(pattern, text, re.DOTALL)

        all_queries = []
        for query, body in matches:
            results = []
            # 在 body 中查找所有编号结果项
            ...
            for rank_str, title, url, snippet in item_pattern.findall(body):
                results.append({
                    "rank": int(rank_str),
                    "title": title.strip(),
                    "url": url.strip(),
                ...
                try:
                    if name =='search':
                        ...
                        try:
                            ...
                            self.search_result_last_round = parsed_result
                        except Exception as e:
                            self.search_result_last_round = None
                    elif name =='visit':
                        if self.search_result_last_round is not None:
                            # 找到URL，双层循环遍历
                            for search_results in self.search_result_last_round:
                                for search_res in search_results['results']:
                                    if search_res.get('url','1')==params.get('url','2'):
                                        params['snippet'] = search_res.get('snippet', '')
                                        break
                            if 'snippet' not in params:
                                params['snippet'] = ''
    ...
    def check_finished(self, infer_request: 'RolloutInferRequest', response_choice: 'ChatCompletionResponseChoice',
        ...
        if total_tokens>26*1024: # 超长了
            ...
            with open('/data/coding/finetuned_model/grpo_final/end_reason.jsonl', 'a') as f:
    ...
    def step(self, infer_request: 'RolloutInferRequest', response_choice: 'ChatCompletionResponseChoice',
        ...
        # tokenizer = self.tokenizer
        # result_tokens = tokenizer.encode("<response>"+(tool_results[0])+"\n</response>", add_special_tokens=False)
