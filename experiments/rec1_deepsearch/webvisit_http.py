...
# ===================== 独立 Visit 网页缓存（纯SQLite） =====================
class VisitSQLCache:
    _lock = threading.Lock()
    _instance: Optional['VisitSQLCache'] = None
    # 独立数据库，不和search缓存混用
    DB_FILE = "visit_webpage_cache.db"

    def __new__(cls) -> 'VisitSQLCache':
    ...
    def get_content(self, url: str) -> Optional[str]:
        with self._lock, sqlite3.connect(self.DB_FILE) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT content FROM webpage_cache WHERE url = ?", (url,))
            res = cursor.fetchone()
    ...
    def set_content(self, url: str, content: str) -> None:
        with self._lock, sqlite3.connect(self.DB_FILE) as conn:
            cursor = conn.cursor()
            cursor.execute("REPLACE INTO webpage_cache (url, content) VALUES (?, ?)",
                          (url, content))
            conn.commit()

# 全局缓存实例
visit_cache = VisitSQLCache()
...
def call_visit_server(msgs, max_tries=3):
    ...
    for attempt in range(max_tries):
        try:
            ...
            if attempt == (max_tries - 1):
                return ""
            time.sleep(1)  # 等待一段时间后重试，避免过快的连续请求
            continue
...
class Visit(BaseTool):
    ...
    def jina_read_page(self,target_url,timeout=20000,enbable_browser=True):
    ...
    #     try:
    #         url = preprocess_url(url) # 有可能是arxiv的，预处理一下，pdf的变成abs的
    #         if '.pdf' in url:
    #             web_page_text = None
    #         else:
    #             _, web_page_text = get_webpage_content(url, if_text_only=True,timeout=20)
    #             if web_page_text is None or len(web_page_text.strip())<1000 or ('IP address' in web_page_text and 'error' in web_page_text and 'Wikimedia System Administrators' in web_page_text):
    #                 web_page_text = self.jina_read_page(url)
    #         if web_page_text:
    ...
    def fetch_url_content(self, url: str):
        # ===================== 【缓存核心】第一步：查缓存 =====================
        cached_content = visit_cache.get_content(url)
        if cached_content is not None:
            #print(f"[Visit缓存命中] URL: {url}")
        ...
        try:
            ...
            if web_page_text:
                # ===================== 【缓存写入】有效内容存入数据库 =====================
                visit_cache.set_content(url, web_page_text)
                #print(f"[Visit缓存写入] URL: {url}")
                return web_page_text
            else:
                return "[visit] Failed to read page because the page is a PDF file."
...
def rollout_visit(params: Union[str, dict], **kwargs) -> str:
    try:
        url = params["url"]
        goal = params["goal"]
    except:
    ...
    if isinstance(url, str):
        response = visit_tool.readpage(url, goal)
    else:
        response = []
        assert isinstance(url, List)
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = {executor.submit(visit_tool.readpage, u, goal): u for u in url}
