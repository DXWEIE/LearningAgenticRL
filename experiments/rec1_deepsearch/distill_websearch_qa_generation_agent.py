...
file_list_zh = ['维基百科中文_历史上的今天_4月.jsonl','维基百科中文_历史上的今天_5月.jsonl']
...
for idx in range(len(file_list_zh)):
    ...
    for days in range(len(wiki_daily_zh)):
        for event in wiki_daily_zh.loc[days,'历史事件']:
            try:
                tmp_title_and_url = []
                for link in event['links']:
                    tmp_title_and_url.append({'title':link['title'], 'href':link['href']})
                generated_seed_q = []
                generated_seed_a = []
                generated_seed_type = []
                gen_with_model_list = []
                ...
                one_line_dict = {}
                one_line_dict['date'] = wiki_daily_zh.loc[days,'日期']
                ...
                # json dump写入jsonl
                with open('seed_source_question_and_answer_zh_45月max.jsonl', 'a') as f:
...
###### 第二步  问题复杂化
df_seed_zh = pd.read_json('seed_source_question_and_answer_zh_45月max.jsonl',lines=True)
...
for i in range(len(df_seed_zh)):
    try:
        ...
        with open('complicated_source_question_and_answer_zh_45月max.jsonl', 'a') as f:
