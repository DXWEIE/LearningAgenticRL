...
# ===================== SQL缓存核心工具类 =====================
class SearchSQLCache:
    _lock = threading.Lock()  # 线程锁，解决多线程缓存冲突
    _instance = None
    DB_FILE = "search_url_cache.db"  # 缓存数据库文件

    def __new__(cls):
        # 单例模式，全局唯一连接
        if cls._instance is None:
            cls._instance = super().__new__(cls)
            cls._instance.init_cache_table()
    ...
    def get(self, url: str) -> str | None:
        ...
        with self._lock, sqlite3.connect(self.DB_FILE) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT content FROM search_cache WHERE url = ?", (url,))
            res = cursor.fetchone()
    ...
    def set(self, url: str, content: str):
        ...
        with self._lock, sqlite3.connect(self.DB_FILE) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "REPLACE INTO search_cache (url, content) VALUES (?, ?)",
                (url, content)
            )
            conn.commit()

# 全局缓存实例
search_cache = SearchSQLCache()
...
def get_iqs_search_result(query,top_k=10):
    ...
    for i in range(min(top_k, len(iqs_response['results']))):
        cached_result = search_cache.get(iqs_response['results'][i]['link'])
        if cached_result is not None:
            pass
        else: # 写入缓存
            search_cache.set(iqs_response['results'][i]['link'], iqs_response['results'][i]['snippet'] + '\n' + iqs_response['results'][i]['mainText'])
...
#     search_tool = Search()
#     test_query = "成龙电影2024"
#     result = web_search.call({"query": [test_query]})
#     print(result)